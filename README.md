 ...
 * Content-Length : 표현 데이터의 길이
  ...
  * 조건부 요청 헤더
  ...
  ### 캐시와 조건부요청 관련 헤더
  #### 캐시란?
  Cache란 **자주 사용하는 데이터나 값을 미리 복사해 놓는 임시장소**라고 할 수 있다. 아래 사진과 같은 저장공간 계층 구조에서 확인 할 수 있듯이 캐시는 `저장공간이 작고 비용이 비싼 대신 빠른 성능을 제공`한다
  <img src="https://img1.daumcdn.net/thumb/R1280x0/?scode=mtistory2&fname=https%3A%2F%2Fblog.kakaocdn.net%2Fdn%2FZdW35%2FbtqCbTDmFj5%2FAKbAUKiWseStnfD1Cd7gqk%2Fimg.png"/>
  * Cache는 아래와 같은 경우 사용하면 용이하다
    * 접근 시간에 비해 원래 데이터를 접근하는 시간이 오래걸리는 경우
    * 반복적으로 동일한 결과를 돌려주는 경우(이미지나 특정 데이터)
  * Cache에 데이터를 미리 복사해 놓으면 계산이나 접근 시간없이 더 빠른 속도로 데이터에 접근할 수 있다.
  * 결국 Cache란 **반복적으로 데이터를 불러오는 경우에 지속적으로 DB혹은 서버에 요청하는것이 아닌 메모리에 데이터를 저장하였다가 불러다 쓰는 것**을 의미한다.
  * 엔터프라이스급 어플리케이션에선 DB의 부하를 줄이고 성능을 높히기 위해 Cache를 사용한다.
  * 원하는 데이터가 캐시에 있는 경우 `Cache Hit`, 원하는 데이터가 캐시에 없는 경우 `Cache Miss`라고 한다
  * 캐시는 저장공간이 작기 떄문에 캐시전략에 따라 저장중인 데이터를 변경해야 한다.
  * 주로 백엔드 개발자는 웹캐시를 많이 사용하게 된다고한다
  * 웹캐시란 클라이언트가 요청하는 Html,image,js,css등에 대해 첫 요청시에 파일을 내려받아 특정 위치에 복사본을 저장하고 이후 동일한 URL의 자원요청을 다시 서버에서 받지 않고 캐시에 저장한 데이터를 사용하고 더 빠르게 서비스 하기 위한 것이다. 서버를 덜 통하니 응답시간이 감소하고 트래픽이 감소해 서버의 성능을 높힐수 있다.
  * 웹 캐시의 종류
    * Browser Caches
      * 브라우저 또는 HTTP요청을 하는 클라이언트에 의해 내부 디스크에 캐시
      * 캐시된 자원을 공유하지 않는한 개인에 한정된 Cache
      * 브라우저의 뒤로가기 버튼 or 이미 방문한 페이지를 재 방문하는 경우에 효과가 좋다.
    * Proxy Caches
      * Browser Caches와 동일한 원리로 동작하며 클라이언트나 서버가 아닌 네트워크 상에서 동작
      * 한정된수의 클라를 위하여 무한대의 웹서버의 컨텐츠를 캐시해 놓는다
    * Gateway Caches 
      * 서버 앞단에 설치되어 요청에 대한 캐시 및 효율적인 분배를 통해 가용성,신뢰성, 성능등을 향상시킨다
      * 무한대의 클라이언트들에게 한정된 수의 웹서버 컨텐츠를 제공한다. 
...
* SSL 통신 절차
 ...
 ## 소켓
 > 소켓이란 네트워크상에서 동작하는 프로그램간 통신의 종착점, 데이터를 받기 위한 창구 역할이라고 한다. 그러므로 프로세스가 데이터를 보내거나 받기 위해선 반드시 소켓을 열어서 소켓에 데이터를 내보내거나 읽어 들여야 한다.
 * 소켓은 프로토콜,ip주소, 포트 넘버로 정의된다
 * 근데 그럼 소켓과 포트 넘버간의 관계가 뭐지?
 > 우선 소켓과 포트의 관계를 설명하기 전 `호스트(Host)`란 **네트워크에 연결되 모든 종류의 장치를 노드(Node)라고 부르는데 노드 중에서도 네트워크 주소(ip)가 할당된 것들을 호스트(Host)라고 한다.** 스마트폰이든 데스크 톱이든 인터넷에 연결되어있으면 네트워크 주소를 할당받은 것이기에 호스트라고 보면 된다. 이 호스트들 끼리 데이터를 주고 받는것.
   * 그런데 실제로 데이터를 주고받는 것은 호스트 내의 프로세스 들이다.
   * 네트워크 공간에서 데이터를 주고 받는 다는 것은 호스트의 프로세스까지 데이터가 오고간다는 것을 의미한다.ex) 카톡을 전송 메세지는 상대 카톡에 도착해야한다. 친구 인스타그램 피드는 내가 확인 할수 있어야한다.
 * 이제 소켓과 포트에 대해서 설명하자면
   * 포트를 다시 알아보자면 상대가 보낸 데이터가 호스트(ip)에 도착했다고 끝이 아닌 호스트의 알맞은 프로세스로 전달되어야 한다. 이를 위해 포트가 등장했다. 
   * 포트는 네트워크를 통해 데이터를 주고받는 프로세스를 식별하기 위해 호스트내부적으로 프로세스가 할당받는 고유한 값이다.
   * 결국 포트 번호는 데이터를 서로 주고받을시 반드시 필요한 정보가 되어 버렸다.
   * 소켓은 프로세스가 네트워크를 통해서 데이터를 주고 받으려면 반드시 열어야 하는 창구 같은 것이다.
   * 프로세스가 상대 프로세스에게 데이터를 보내려면 내 소켓도 열어야 하지만 상대 소켓도 열려있어야 전송이 가능하다.
   * 하지만 소켓과 포트는 다르다 왜냐면 하나의 프로세스는 보통 하나의 포트를 할당 받는데 이 프로세스는 하나의 포트를 가지고 여러개의 소켓을 열수 있다고 한다
   * 이렇게 같은 프로토콜,ip주소,포트넘버 를가지고 여러개의 소켓을 가질수 있다. 이런이유로 하나의 프로세스는 하나의 포트만으로 여러 호스트의 프로세스의 요청을 처리할수 있다. ex) 게임서버 동접수 많은 것
   * 그렇다면 같은 포트 넘버에 여러 소켓이 있다고 했는데 이들에 대한 구별은 어떻게 할까?
     * 소켓을 구별할때에는 보낸쪽의 ip주소, 포트번호, 받는 쪽의 ip주소,포트번호가 하나의 소켓에 매칭되고 이후로 주고 받는 모든 데이터는 4가지 필드가 일치하는 소켓으로 데이터 통신이 이루어 진다고 한다. 
 즉 정리하자면
 > 포트는 네트워크 상에서 통신하기 위해 호스트 내부적으로 프로세스가 할당 받아야 하는 고유한 숫자이다. 한 호스트내에서 네트워크 통신을 하고 있는 프로세스를 식별하기 위해 사용되는 값이므로 기본적으로 같은 호스트 내에서 서로 다른 프로세스가 같은 포트 넘버를 가질 수 없다(보통은? 방법이 있긴하다)
 >  소켓은 프로세스가 넓은 네트워크 세계로 데이터를 내보내거나 혹은 그 세계로부터 데이터를 받기 위한 실제적인 창구역할을 한다. 그러므로 프로세스가 데이터를 보내거나 받기 위해서는 반드시 소켓을 열어서 소켓에 데이터를 써보내거나 소켓으로부터 데이터를 읽어들여야 한다.
